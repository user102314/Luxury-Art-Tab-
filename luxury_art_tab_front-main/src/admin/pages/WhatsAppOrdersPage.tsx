...
import type { WhatsAppOrderLine, OrderStatut } from '../types'

const STATUTS: OrderStatut[] = ORDER_STATUTS

const emptyLine = (): WhatsAppOrderLine => ({ productId: 0, quantite: 1 })
...
export default function WhatsAppOrdersPage() {
  ...
  const [referenceWhatsapp, setReferenceWhatsApp] = useState('')
  const [statut, setStatut] = useState<OrderStatut>('EN_ATTENTE')
  const [lines, setLines] = useState<WhatsAppOrderLine[]>([emptyLine()])
  ...
  const whatsappOrders = useMemo(
    () =>
      filterSortOrders(
        orders.filter((o) => o.canal === 'WHATSAPP'),
  ...
  const updateLine = (index: number, patch: Partial<WhatsAppOrderLine>) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await api.createWhatsAppOrder({
        clientNom,
        clientEmail: clientEmail || undefined,
        clientTelephone: clientTelephone || undefined,
        adresseLivraison: adresse,
        referenceWhatsapp: referenceWhatsapp || undefined,
        statut,
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500">
            Saisie manuelle des ventes WhatsApp — stock et revenus mis à jour automatiquement
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          value={String(channelStats?.totalWhatsapp ?? whatsappOrders.length)}
          subtitle={`${channelStats?.whatsappLivrees ?? 0} livrées (CA)`}
          icon={MessageCircle}
          accent="purple"
        />
        <StatCard
          title="CA WhatsApp"
          value={formatCurrency(Number(channelStats?.caWhatsapp) || 0)}
      ...
      <form onSubmit={handleSubmit} className="card space-y-5 p-6">
        <h3 className="font-semibold text-white">Nouvelle commande WhatsApp</h3>

        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <label className="label">Nom client WhatsApp *</label>
          ...
          <div>
            <label className="label">Réf. conversation WhatsApp</label>
            <input
              className="input"
              value={referenceWhatsapp}
              onChange={(e) => setReferenceWhatsApp(e.target.value)}
              placeholder="Lien ou n° WhatsApp / conversation"
            />
          </div>
        </div>
              ...
              <div key={index} className="flex flex-wrap items-end gap-3 rounded-xl bg-ink-800/50 p-4">
                ...
                {product && (
                  <p className="pb-2 text-xs text-zinc-500">
                    ...
                    <strong
                      className={
                        product.stock - line.quantite < 0 ? 'text-red-400' : 'text-emerald-400'
                      }
                    >
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-4">
          ...
          <button type="submit" disabled={saving} className="btn-primary">
            {saving ? 'Enregistrement…' : 'Enregistrer la commande WhatsApp'}
          </button>
        </div>
      ...
      <ListToolbar
        ...
        searchPlaceholder="Rechercher client, n° commande, réf. WhatsApp…"
        sort={sort}
        ...
        resultCount={whatsappOrders.length}
        totalCount={orders.filter((o) => o.canal === 'WHATSAPP').length}
        onReset={() => {
          setSearch('')
          setListStatut('ALL')
          setSort('date')
      ...
      <div className="card overflow-hidden">
        <div className="border-b border-white/10 px-6 py-4">
          <h3 className="font-semibold text-white">Historique WhatsApp ({whatsappOrders.length})</h3>
        </div>
        {whatsappOrders.length === 0 ? (
          <p className="p-8 text-center text-zinc-500">Aucune commande WhatsApp enregistrée</p>
        ) : (
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-white/10 text-xs uppercase text-zinc-500">
                  ...
                  <th className="px-6 py-3">Réf. WA</th>
                </tr>
              </thead>
              <tbody>
                {whatsappOrders.map((o) => (
                  <tr key={o.id} className="border-b border-white/5">
                    ...
                    <td className="px-6 py-3">
                      ...
                      {o.clientTelephone && (
                        <p className="text-xs text-zinc-500">{o.clientTelephone}</p>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-3 font-semibold text-gold-400">
                    ...
                    <td className="max-w-[120px] truncate px-6 py-3 text-xs text-zinc-500">
                      {o.referenceWhatsapp ?? '—'}
                    </td>
                  </tr>
                ))}
