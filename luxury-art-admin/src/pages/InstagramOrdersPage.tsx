...
export default function InstagramOrdersPage() {
  ...
  const [referenceInstagram, setReferenceInstagram] = useState('')
  ...
  const updateLine = (index: number, patch: Partial<InstagramOrderLine>) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await api.createInstagramOrder({
        clientNom,
        clientEmail: clientEmail || undefined,
        clientTelephone: clientTelephone || undefined,
        adresseLivraison: adresse,
        referenceInstagram: referenceInstagram || undefined,
        statut,
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500">
            Saisie manuelle des ventes Instagram — stock et revenus mis à jour automatiquement
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          title="Commandes Instagram"
          value={String(channelStats?.totalInstagram ?? instagramOrders.length)}
          subtitle={`${channelStats?.instagramLivrees ?? 0} livrées (CA)`}
          icon={Instagram}
          accent="purple"
        />
        <StatCard
          title="CA Instagram"
          value={formatCurrency(Number(channelStats?.caInstagram) || 0)}
      ...
      <form onSubmit={handleSubmit} className="card space-y-5 p-6">
        <h3 className="font-semibold text-white">Nouvelle commande Instagram</h3>

        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <label className="label">Nom client Instagram *</label>
          ...
          <div>
            <label className="label">Réf. conversation Instagram</label>
            <input
              className="input"
              value={referenceInstagram}
              onChange={(e) => setReferenceInstagram(e.target.value)}
              placeholder="Lien ou @pseudo / n° DM"
            />
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-4">
          ...
          <button type="submit" disabled={saving} className="btn-primary">
            {saving ? 'Enregistrement…' : 'Enregistrer la commande Instagram'}
          </button>
        </div>
      ...
      <div className="card overflow-hidden">
        <div className="border-b border-white/10 px-6 py-4">
          <h3 className="font-semibold text-white">Historique Instagram ({instagramOrders.length})</h3>
        </div>
        {instagramOrders.length === 0 ? (
          <p className="p-8 text-center text-zinc-500">Aucune commande Instagram enregistrée</p>
        ) : (
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-white/10 text-xs uppercase text-zinc-500">
                  ...
                  <th className="px-6 py-3">Réf. IG</th>
                </tr>
              </thead>
              <tbody>
                {instagramOrders.map((o) => (
